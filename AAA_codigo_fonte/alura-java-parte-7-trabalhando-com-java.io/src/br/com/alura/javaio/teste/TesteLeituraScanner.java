		...
		try (Scanner sc = new Scanner(new File("contas.csv"))) {
			...
			while (sc.hasNextLine()) {
				...
				try (Scanner linhaScanner = new Scanner(linha)) {
					...
					String valorFormatado = String.format(new Locale("pt", "BR"),"%s ; %s ; %s ; %s ; %4.2f ; %b \\ \n \t \r ", tipoConta, agencia,
							conta, nomeTitular, saldo, false);
					System.out.println(valorFormatado);
					System.out.printf(new Locale("pt", "BR"),"Tipo da Conta: %s Agencia: %04d Conta: %010d -> Titular: %20s - > Saldo : %010.2f ; %b \\ \n \t \r ", 
							tipoConta, agencia, conta, nomeTitular,saldo, false);
					System.out.format(new Locale("pt", "BR"),"Tipo da Conta: %s Agencia: %04d Conta: %010d -> Titular: %20s - > Saldo : %010.2f ; %b \\ \n \t \r ", 
							tipoConta, agencia, conta, nomeTitular,saldo, false);
